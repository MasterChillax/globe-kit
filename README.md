...
- **라벨식은 "한국어 먼저"의 완전한 뜻**(v0.2.6, Corea 실측): `labelTextField()` = `name:ko` → **한글 `name`** → `name:latin` → `name`. name:ko 없는 한국 지물(서울 z14 길 5/391·장소 1/203·물 이름 2/11)이 로마자("Yulgok-ro 5-gil")로 나오던 것을 막는다. MapLibre style-spec 으로 유효성·평가를 테스트에서 고정.
- **스타일 URL 스위치 게이트**(v0.2.6): https 만, URL 정규화(소문자 호스트·기본 포트 제거), 키 있는 공급자는 **서브도메인까지** 판정(`tiles.basemaps.cartocdn.com` 도 CARTO → 키 없으면 throw), `{ exposure: 'public' }` 이면 레지스트리에 없는 호스트 거부(처리방침이 수신자를 명시해야 하므로). private 앱은 자체 호스팅 허용(린트가 `unregistered-host` 로 알림).
- **라벨 치환 범위**(v0.2.5, Corea 실측): `localizeLabels` 는 `readsName(text-field)` 인 층만 바꾼다 — `name`·`name:*`·`name_*` 를 읽는 층. OFM dark 의 `highway_name_motorway`(`["to-string",["get","ref"]]`, 고속도로 번호)·화살표·고정 문자열은 그대로.
- **바다 이름 paint**(v0.2.5): `seaNameLayerFrom` 은 템플릿 paint 를 물려받지 않는다 — OFM dark 의 `water_name` 은 검정 0.7·halo 0 이라 물 면 위에서 1.16:1(그려지지만 안 보임). 기본은 `SEA_NAME_PAINT_DARK`(#a8b9ac, halo 1 — 물 위 약 8.4:1), `opts.paint` 로 덮어쓴다. 스크린샷으로 확인할 것 — `queryRenderedFeatures` 는 대비를 모른다.
- **등록 호스트는 서브도메인 포함**(v0.2.5): CARTO 스타일의 소스(`tiles.basemaps.cartocdn.com`)는 `basemaps.cartocdn.com` 등록으로 통과(라벨 경계만, 접미 유사 호스트 `…cartocdn.com.example.net` 은 여전히 `unregistered-host`).
- **`stack.dropped`**(v0.2.3): 프로필이 못 싣는 공급자만, `{ id, reason, kind, role }`. 백업 스타일(`fallback: true`)은 키가 없어도 여기 안 들어온다 — 진단 줄에 "백업: 키 없음"을 띄울 이유가 없어서. 앱은 `role`(satellite·korea-satellite·…)로 문구를 고른다.
